...
namespace BezierUtils
{
  ...
  namespace detail
  {
    ...
    struct EvaluateCurvePoint
    {
      ...
      static inline Pts call(PtsIt const beg, PtsIt const fin, T const &u)
      {
  ...
  template<typename PtsIt, typename WtsIt, typename NonRatIt>
  void transformToHomog(PtsIt begP, PtsIt const endP, WtsIt begW, NonRatIt begO)
  {
    ...
    while (begP != endP)
    {
      ...
      *(begO->end() - 1) = *begW;
      ...
      std::for_each(
        begin(*begO), end(*begO) - 1, [&begW](T &c) { c *= *begW; });
